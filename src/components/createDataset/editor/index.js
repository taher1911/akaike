...
import "../../../styles/createDataset/editor.css";
...
const ReactPictureTagger = ({
  ...
  const tagColors = ["#20629B", "#F6D55C", "#3CAEA3", "#ED553B", "#173f5f"];
  ...
  function mouseUp() {
    if (drag) {
      ...
      setEditingTag({
        index: null,
        left: Math.round(
          (rect.startX > rect.finishX ? rect.finishX : rect.startX) /
            resizeRatio
        ),
        top: Math.round(
          (rect.startY > rect.finishY ? rect.finishY : rect.startY) /
            resizeRatio
        ),
        width: Math.round(
          (rect.startX > rect.finishX
            ? rect.startX - rect.finishX
            : rect.finishX - rect.startX) / resizeRatio
        ),
        height: Math.round(
          (rect.startY > rect.finishY
            ? rect.startY - rect.finishY
            : rect.finishY - rect.startY) / resizeRatio
        ),
        name: "",
      });
      setAddTagMode(false);
  ...
  return (
    <div
      className="reactPictureTagger"
      style={addTagMode ? { cursor: "copy" } : null}
    >
      <div className="reactPictureTagger-tagControls">
        <a onClick={toggleTags} title={tagsVisible ? "Hide tags" : "Show tags"}>
          {tagsVisible ? <VisibilityOffOutlined /> : <VisibilityOutlined />}
        </a>
      ...
      <div className="reactPictureTagger-pictureContainer">
        <canvas width={720} ref={canvasRef}></canvas>
        {tagsVisible
          ? allTags.map((tag, index) => {
              ...
              return !editingTag ? (
                <div
                  onClick={() => editTag(index, tag)}
                  key={`tag-${index}`}
                  className="reactPictureTagger-tag"
                  style={tagStyle}
                >
                  <span
                    className="reactPictureTagger-tagName"
                    style={tagNameStyle}
                  >
                    {tag.name}
                  </span>
